...
class OptimHIM(HIMBase):
    def __init__(
        ...
        **kwargs
        ...
        self.B = nn.Parameter(torch.ones(N)*0.01)  # (N, ) diagonal of B matrix

        self.register_buffer('A', torch.eye(N).unsqueeze(
            0).unsqueeze(1))    # (1, 1, N, N)
        self.register_buffer('Q', torch.eye(N).unsqueeze(
            0).unsqueeze(1) * Q)    # (1, 1, N, N)
        self.register_buffer('R', torch.eye(N).unsqueeze(
            0).unsqueeze(1) * R)    # (1, 1, N, N)

    def forward(self, x):
        B = torch.diag_embed(self.B)    # (N, N)
        B = B.unsqueeze(0).unsqueeze(0) # (1, 1, N, N)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Predict B
        B, _ = self.forward(batch)

        # Get P_H via DARE
        P = self.dare(self.A, B, self.Q, self.R) # (1, 1, N, N)
        P = P.expand(batch_size, seq_len, *P.shape[-2:]) # (B, T, N, N)
        ...
        x_next_star = self.dynamics(self.A, B, x, u_H_star)
        x_next_hat = self.dynamics(self.A, B, x, u_H_hat)

        # Compute Q-function values
        Q_u_H = self.value_Q(
            P, self.Q, self.R, x, u_H_hat, x_next_hat)   # (B, T)
        Q_u_H_star = self.value_Q(
            P, self.Q, self.R, x, u_H_star, x_next_star)  # (B, T)

        # Compute likelihoods loss (maximum)
        likelihoods = self.likelihood_u_H(
            P, B, self.R, Q_u_H - Q_u_H_star)  # (B, T)
        loss = - likelihoods.sum(dim=-1).mean()

        # Log `B` (vector) in a logger-friendly way: mean + per-element scalars
        logs = {"B_mean": self.B.mean()}
        for i in range(self.B.numel()):
            logs[f"B_{i}"] = self.B[i]
        self.log_dict(logs, on_epoch=True, on_step=False, logger=True)
